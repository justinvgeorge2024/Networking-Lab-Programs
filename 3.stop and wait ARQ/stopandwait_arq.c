// stop and wait ARQ Protocol implementation [in c as a single program.]
...
void sender(){
	...
	if(turn=='s'){
		if(errorack==0){
			...
			if(flag==1) printf("SENDER : Recieved ACK from packet %d\n",ak);
			if(p.seq==5){
				disconnect=1;
				return;
			}
...
void receiver(){
	...
	if(turn='r'){
		if(errorframe!=0){
			if(p.seq==frexp){
				printf("RECEIVER: recieved packet with seq no: %d\n",p.seq);
				ak=p.seq;
				frexp=frexp+1;
				turn='s';
				errorack=rand()%4;
